import { NextApiRequest, NextApiResponse } from 'next';
import fs from 'fs/promises';
import path from 'path';
import { scrapeAll, URLS } from '../../scrape';

type SalaryEntry = {
	'annual-rates-of-pay': Array<{
		'effective-date': string | null;
		[key: string]: string | number | null | undefined;
	}>;
};

type SalaryData = Record<string, SalaryEntry>;

type GroupSummary = {
	group: string;
	total: number;
	codes: string[];
};

const buildGroupSummary = (data: SalaryData): GroupSummary[] => {
	const summary = new Map<string, Set<string>>();

	for (const rawCode of Object.keys(data)) {
		if (!rawCode) continue;

		const normalized = rawCode.toUpperCase();
		const match = normalized.match(/^[A-Z]+/);
		const group = match ? match[0] : normalized;

		if (!summary.has(group)) {
			summary.set(group, new Set());
		}

		summary.get(group)!.add(normalized);
	}

	return Array.from(summary.entries())
		.map(([group, codes]) => ({
			group,
			total: codes.size,
			codes: Array.from(codes).sort((a, b) => a.localeCompare(b))
		}))
		.sort((a, b) => a.group.localeCompare(b.group));
};

// Helper to detect if we're running in a serverless environment
function isServerless(): boolean {
	return !!(
		process.env.LAMBDA_TASK_ROOT ||
		process.env.AWS_LAMBDA_FUNCTION_NAME ||
		process.env.VERCEL ||
		process.env.NETLIFY
	);
}

export default async function handler(req: NextApiRequest, res: NextApiResponse) {
	if (req.method !== 'POST') {
		res.setHeader('Allow', 'POST');
		return res.status(405).json({ error: 'Method not allowed' });
	}

	try {
		console.log('[scraper] Starting canonical scraper run');
		console.log(`[scraper] Processing ${URLS.length} Treasury Board URLs`);
		console.log(`[scraper] Environment: ${isServerless() ? 'serverless' : 'local'}`);

		const scraped = await scrapeAll(URLS);
		const processedCount = Object.keys(scraped).length;
		console.log(`[scraper] Successfully parsed ${processedCount} classifications from source pages`);

		if (processedCount === 0) {
			console.warn(
				'[scraper] WARNING: No classifications were found. This might indicate a problem with the source URLs or parsing logic.'
			);
		}

		const dataPath = path.join(process.cwd(), 'data', 'data.json');

		let existingData: SalaryData = {};
		let mergedData: SalaryData = scraped;
		let newCodes: string[] = [];

		// In serverless environments, skip file operations (read-only file system)
		if (!isServerless()) {
			try {
				const raw = await fs.readFile(dataPath, 'utf8');
				existingData = raw ? (JSON.parse(raw) as SalaryData) : {};
				console.log(`[scraper] Loaded ${Object.keys(existingData).length} existing classifications from data.json`);
			} catch (readError) {
				console.warn('[scraper] Existing data could not be read. A new data.json will be created.', readError);
				existingData = {};
			}

			const existingKeySet = new Set(Object.keys(existingData).map((code) => code.toUpperCase()));
			newCodes = Object.keys(scraped).filter((code) => !existingKeySet.has(code.toUpperCase()));

			mergedData = { ...existingData, ...scraped };

			await fs.writeFile(dataPath, JSON.stringify(mergedData, null, 2), 'utf8');
			console.log(
				`[scraper] data.json updated. Persisted total: ${Object.keys(mergedData).length}. New classifications: ${
					newCodes.length
				}.`
			);
		} else {
			console.log(`[scraper] Serverless environment - skipping file operations. Using scraped data directly.`);
			// In serverless, all scraped codes are "new" since we don't persist
			newCodes = Object.keys(scraped);
		}

		const persistedTotal = Object.keys(mergedData).length;
		const updatedAt = new Date().toISOString();

		const groupSummary = buildGroupSummary(mergedData);

		return res.status(200).json({
			success: true,
			message: `Scraper run completed at ${updatedAt}`,
			processedClassifications: processedCount,
			newClassifications: newCodes.length,
			newCodes,
			persistedTotal,
			updatedAt,
			urlsProcessed: URLS.length,
			data: scraped,
			groupSummary
		});
	} catch (error) {
		console.error('[scraper] Scraper error:', error);

		// Provide more detailed error information
		const errorDetails = {
			message: error instanceof Error ? error.message : 'Unknown error',
			stack: error instanceof Error ? error.stack : undefined,
			name: error instanceof Error ? error.name : 'UnknownError'
		};

		return res.status(500).json({
			success: false,
			error: 'Scraping failed',
			details: errorDetails.message,
			debug: process.env.NODE_ENV === 'development' ? errorDetails : undefined,
			timestamp: new Date().toISOString()
		});
	}
}
